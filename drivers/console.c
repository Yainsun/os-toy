...
static void scroll()
{
	...
	uint16_t blank = 0x20 | (attribute_byte << 8);  // space 是 0x20, 另外注意这里是两个字节，我们使用的qemu i386是小端系统，所以低字节在前，表示字符空格，高字节在后，表示颜色
...
void console_clear()
{
	uint8_t attribute_byte = (0 << 4) | (15 & 0x0F); // 颜色值 0000 1111 // 黑底白字， 但是空格本身不打印字符，所以看起来就是黑底无字了
	uint16_t blank = 0x20 | (attribute_byte << 8); // space 是 0x20, 另外注意这里是两个字节，我们使用的qemu i386是小端系统，所以低字节在前，表示字符空格，高字节在后，表示颜色
	// uint8_t attribute_byte = (3 << 6) | (14 & 0x0F); // 颜色值 1100 1110 // 红底黄字
	// uint16_t blank = 0x4F | (attribute_byte << 8); // 使用大写字母 'O' 作为填充
